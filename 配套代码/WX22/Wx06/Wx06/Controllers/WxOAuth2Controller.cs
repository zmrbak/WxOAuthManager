...
namespace WxOAuthManager.Controllers
{
    ...
    public class WxOAuth2Controller : ControllerBase
    {
        ...
        [HttpGet]
        [Route("/WeChatScan.aspx")]
        public IActionResult WeChatScan()
        {
            string redirect_uri = UrlWithoutPath()+ "/LoginCallback.aspx";

            var scope = "snsapi_login";// configuration["WeChatMpProxy:Scope"];
            var state = configuration["WeChatMpProxy:State"];
            var proxy = configuration["WeChatMpProxy:Proxy"];
            var url = proxy + $"/connect/oauth2/qrconnect?redirect_uri={HttpUtility.UrlEncode(redirect_uri)}&scope={scope}&state={state}";
            return Redirect(url);
        }
